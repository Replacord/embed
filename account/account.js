...
dbRef.get().then((snapshot) => {
  ...
  let bioo = data.bio
  let biotext = bioo.replaceAll('%domain%', document.domain).replaceAll('%url%', document.URL).replaceAll('%username%', username).replaceAll('%urlparams%', window.location.search).replaceAll('(', '<a href=>').replaceAll(')', '>').replaceAll('[', '').replaceAll(']', '</a>').replaceAll('{', '<button style=""form-btn-h>').replaceAll('}', '</button>')
